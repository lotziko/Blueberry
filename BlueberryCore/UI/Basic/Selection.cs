...
namespace BlueberryCore.UI
{
    public class Selection<T> where T : class
    {
        ...
        public virtual void Choose(T item)
        {
            ...
            try
            {
                if ((toggle || (!required && selected.Count == 1) || InputUtils.IsCtrlDown()) && selected.Contains(item))
                {
                ...
                {
                    bool modified = false;
                    if (!multiple || (!toggle && !InputUtils.IsCtrlDown()))
                    {
        ...
        public Selection<T> Set(T item)
        {
            ...
            if (selected.Count == 1 && selected.First().Equals(item))
                return this;
