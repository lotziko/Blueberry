...
namespace BlueberryCore.UI
{
    public class Table : Group
    {
        ...
        public Table(string label, Skin skin) : this()
        {
        ...
        public override Element Hit(Vector2 point, bool touchable)
        {
            if (clip)
            {
                if (touchable && GetTouchable() == Touchable.Disabled) return null;
                if (point.X < 0 || point.X >= GetWidth() || point.Y < 0 || point.Y >= GetHeight()) return null;
            }
        ...
        public override void DrawDebug(Graphics graphics)
        {
            if (debugRects != null)
            {
                foreach (var d in debugRects)
                    graphics.DrawRectangleBorder(x + d.rectange.X, y + d.rectange.Y, d.rectange.Width, d.rectange.Height, d.color);
            }
    ...
    public struct DebugRectangle
    {
        ...
        public DebugRectangle(float x, float y, float w, float h, Color color)
        {
            rectange = new Rectangle((int) x, (int) y, (int) w, (int) h);
            this.color = color;
        }
    }
