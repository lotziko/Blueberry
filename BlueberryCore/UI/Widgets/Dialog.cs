    ...
    public class Dialog : Window
    {
        ...
        private class Focus : FocusListener
        {
            ...
            private void FocusChanged(FocusEvent ev)
            {
                ...
                if (d.IsModal() && stage != null && stage.GetRoot().GetElements().Count > 0
                        && stage.GetRoot().GetElements().Peek() == d) { // Dialog is top most actor.
        ...
        {
            Show(stage, SAction.Sequence(SAction.Alpha(0), SAction.FadeIn(0.4f, Interpolation.fade)));
        ...
        public void Hide(Action action)
        {
            ...
            if (action != null)
            {
                ...
                AddAction(SAction.Sequence(action, SAction.RemoveListener(ignoreTouchDown, true), SAction.RemoveElement()));
            }
            else
                Remove();
        }
        ...
        public void Hide()
        {
            Hide(SAction.Sequence(SAction.FadeOut(FADE_TIME, Interpolation.fade), SAction.RemoveListener(ignoreTouchDown, true), SAction.RemoveElement()));
        }
