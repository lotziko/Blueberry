...
namespace BlueberryCore.UI
{
    public class Tree : Group
    {
        ...
        protected void DrawBackground(Graphics graphics, float parentAlpha)
        {
            ...
            {
                style.background.Draw(graphics, x, y, width, height, new Color(color, color.A * parentAlpha));
            }
        }
        ...
        private class Listener : ClickListener
        {
            ...
            public override void Clicked(InputEvent ev, float x, float y)
            {
                ...
                if (_t.selection.GetMultiple() && _t.selection.HasItems() && InputUtils.IsShiftDown())
                {
                    // Select range (shift).
                    if (_t.rangeStart == null) _t.rangeStart = node;
                    Node rangeStart = _t.rangeStart;
                    if (!InputUtils.IsCtrlDown()) _t.selection.Clear();
                ...
                if (node.children.Count > 0 && (!_t.selection.GetMultiple() || !InputUtils.IsCtrlDown()))
                {
