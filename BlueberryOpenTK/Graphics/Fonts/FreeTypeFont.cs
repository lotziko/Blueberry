...
namespace BlueberryOpenTK
{
    public class FreeTypeFont : IFont, IDisposable
    {
        ...
        const float ascentScale = 0.9f;

        public FreeTypeFont(string path, float size)
        {
            float dpi = 1f;
            ...
            while (true)
            {
                ...
                if (width == 0 || rect.width != 0)
                {
                    ...
                    avalibleGlyphs.Add(character, new Glyph()
                    {
                        ...
                        xadvance = Math.Max(advance, width + xOffset),
                        yadvance = 0,
                        xbearing = xOffset,
                        ybearing = yOffset
                    });
        ...
        public void Draw(Graphics graphics, string text, float x, float y, Col? color = null)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                tmpGlyph.region.Draw(graphics, pos.X + tmpGlyph.xbearing, pos.Y + tmpGlyph.ybearing + Ascent * ascentScale, color);
                //graphics.DrawRectangle(pos.X, pos.Y + tmpGlyph.ybearing + Ascent, tmpGlyph.region.Width, tmpGlyph.region.Height, true, Col.Red);
        ...
        public Vec2 MeasureString(string text)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                var proposedWidth = offset.X;// + glyph.preferredWidth;// - glyph.xbearing;// + spacing + padLeft + padRight;
                //if (proposedWidth > width)
                    width = proposedWidth;
        ...
        {
            return font.FindGlyphIndex(character) != 0;
        }
