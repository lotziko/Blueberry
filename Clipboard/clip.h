...
namespace clip {
	...
	extern "C" typedef size_t format;
	...
	__declspec(dllexport) bool set_text(const std::string& value);
	__declspec(dllexport) bool get_text(std::string& value);
	...
	public:
		...
		__declspec(dllexport) image(const image_spec& spec);
		__declspec(dllexport) image(const void* data, const image_spec& spec);
		__declspec(dllexport) image(const image& image);
		__declspec(dllexport) image(image&& image);
		__declspec(dllexport) ~image();
	...
	// functions returns false in case of error.
	__declspec(dllexport) bool set_image(const image& img);
	__declspec(dllexport) bool get_image(image& img);
	__declspec(dllexport) bool get_image_spec(image_spec& spec);
